        ...
        result = await self.session.execute(
            text("""
                WITH RECURSIVE cat_tree AS (
                    SELECT id, nombre, descripcion, parent_id, creado_en, actualizado_en, 1 as nivel
                    FROM categoria WHERE parent_id IS NULL AND deleted_at IS NULL
                    UNION ALL
                    SELECT c.id, c.nombre, c.descripcion, c.parent_id, c.creado_en, c.actualizado_en, ct.nivel + 1
                    FROM categoria c
                    INNER JOIN cat_tree ct ON c.parent_id = ct.id
                    WHERE c.deleted_at IS NULL
                )
                SELECT id, nombre, descripcion, parent_id, creado_en, actualizado_en, nivel
